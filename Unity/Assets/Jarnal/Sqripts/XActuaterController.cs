...
public class XActuatorController : MonoBehaviour
{
    ...
    public String Host = "192.168.16.7";
    public Int32 Port = 12345;
    public GameObject targetObject;  // 対象のGameObject
    public String messageToSend = "0";
    public float sendInterval = 0.1f;  // 60fpsで送信
    private float previousXAngle = 0f;      // 前回のX角度

    void Start()
    {
        setupSocket();
        previousXAngle = GetXAngle();  // 初期角度取得
    ...
    void SendMessage()
    {
        ...
        //{
            ...
            {
                // 現在のZ軸回転取得
                float currentXAngle = GetXAngle();

                // 差分計算（-180〜180度範囲内での差）
                float angleDiff = Mathf.DeltaAngle(previousXAngle, currentXAngle);

                // -50〜50 にマッピング（最大180度の時 ±50 になるようにスケーリング）
                float mappedDiff = Mathf.Clamp(angleDiff / 3.6f, -45f, 50f) * 100;
